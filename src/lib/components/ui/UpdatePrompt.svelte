...
{#if $needRefresh}
	<div
		class="fixed top-0 right-0 left-0 z-50 border-b border-[var(--color-gold)] bg-[var(--color-brand)] px-4 py-3"
	...
	>
		<div class="mx-auto flex max-w-[480px] items-center gap-3">
			...
			<button
				onclick={update}
				class="tap-target shrink-0 rounded-lg bg-white px-4 text-sm font-semibold text-[var(--color-brand)]"
			>
				Perbarui
			</button>
		</div>
